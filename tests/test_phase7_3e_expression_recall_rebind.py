...
def _teach_multi_reply(state: dict, runtime: IncrementalTickRuntime) -> dict:
    for step in tuple(
        _teach(
            tick,
            case_name="phase7_3e_multi_reply",
            cue_tokens=(SAN, GU),
            reply_tokens=(MAO, LU),
...
def _teach_expression_paradigm(state: dict, runtime: IncrementalTickRuntime) -> dict:
    for step in (
        _teach(
            70,
            case_name="phase7_3e_expression",
...
def _seed_introspection_and_association(state: dict) -> dict:
...
def _expression_pid(state: dict) -> str:
    for row in state["paradigms"]:
        if "phase7_3e_expression" in row.get("probe_tags", []):
            return str(row["pid"])
    raise AssertionError("expression paradigm not found")


def _recall_with_query(tick: int = 100) -> IncrementalTickInput:
    return IncrementalTickInput(
        tick=tick,
        cue_tokens=(SAN, GU),
        context_tokens=("ctx_idiom",),
...
def test_phase7_3e_external_query_uses_learned_feeling_expression_association() -> None:
    runtime = IncrementalTickRuntime()
    state = _seed_introspection_and_association(_teach_expression_paradigm(_teach_multi_reply(_base_state(), runtime), runtime))

    result = runtime.run_tick(state, _recall_with_query())
    ...
    assert any(item["tick"] == 100 for item in result.dialogue_result.state.get("introspection_feelings", []))


def test_phase7_3e_query_without_association_keeps_undecidable_draft_uncommitted() -> None:
    runtime = IncrementalTickRuntime()
    state = _teach_expression_paradigm(_teach_multi_reply(_base_state(), runtime), runtime)
...
def test_phase7_3e_association_without_reply_pressure_does_not_force_expression() -> None:
    runtime = IncrementalTickRuntime()
    state = _seed_introspection_and_association(_teach_expression_paradigm(_teach_multi_reply(_base_state(), runtime), runtime))
